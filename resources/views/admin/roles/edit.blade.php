...
@section('title', 'แก้ไขบทบาท - CMC-STOCK')
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">แก้ไขบทบาท</li>
            </ol>
        </div>
    </div>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">แก้ไขข้อมูลบทบาท</h3>
        </div>
        ...
        <form action="{{ route('admin.roles.update', $role) }}" method="POST">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('level') is-invalid @enderror" id="level" name="level" required>
                                ...
                                <option value="1" {{ old('level', $role->level) == 1 ? 'selected' : '' }}>1 - Master Admin</option>
                                <option value="2" {{ old('level', $role->level) == 2 ? 'selected' : '' }}>2 - Admin</option>
                                <option value="3" {{ old('level', $role->level) == 3 ? 'selected' : '' }}>3 - Member</option>
                            </select>
                            @error('level')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              id="description" name="description" rows="3" placeholder="คำอธิบายเกี่ยวกับบทบาทนี้">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="card-footer">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> อัปเดตบทบาท
                </button>
